...
async function getProducts(params, callback) {
    ...
    product.find(condition, "productName productShortDescription productDescription productPrice productSalePrice productImage productSKU productType stockStatus")
...
async function deleteProduct(params, callback) {
    ...
    product.findByIdAndRemove(productId)
        .then((response) => {
            if (!response) callback(`Cannot delete Product with ID ${productId}`);
            else callback(null, response);
        })
        .catch((error) => {
            return callback(error);
